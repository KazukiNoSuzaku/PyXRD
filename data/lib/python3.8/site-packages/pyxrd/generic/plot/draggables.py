...
class DraggableMixin(object):
    ...
    Replaces the GTK version that used Gdk.Cursor / GdkWindow.set_cursor.
    """
    ...
    def _draggable_on_motion(self, event):
        ...
        if callable(self._on_dragged):
            self._on_dragged((x0, y0), (x1, y1), (x2, y2), event=event)
    ...
    def _draggable_on_release(self, event):
        ...
        if callable(self._on_released):
            self._on_released((x0, y0), (x1, y1), (x2, y2), event=event)
...
class DraggableVLine(DraggableMixin):
    ...
    `window` is expected to be a QWidget (the matplotlib canvas or its parent).
    """
    ...
    def __init__(self, line, callback=None, window=None):
        ...
        self.window = window  # QWidget for cursor changes
