...
if TOOLKIT == "gtk":
    ...
    def _get_widget_name(wid):
        try:
            return Gtk.Buildable.get_name(wid)
        except TypeError:
            return None
    ...
    from .adapters.qt_support.builder import QtBuilder
    Builder = QtBuilder

    def _get_widget_name(wid):
        return wid.objectName() if hasattr(wid, 'objectName') else None

    def _show_top(top):
        top.show()

else:
    raise ImportError("Unknown TOOLKIT: %s" % TOOLKIT)
    ...
    def show(self):
        ...
        if type(top) in (list, tuple):
            for t in top:
                ...
                pass
        elif (top is not None): _show_top(top)
        return
